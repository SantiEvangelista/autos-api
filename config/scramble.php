...
La API sigue una jerarquía de 4 niveles:

```
Marca (Brand) → Modelo (CarModel) → Versión (Version) → Valuación (Valuation)
```

- **Marca**: Ej. TOYOTA, FORD, CHEVROLET.
- **Modelo**: Ej. COROLLA, HILUX, RANGER.
- **Versión**: Variante específica de un modelo. Ej. "4P 2.0 XEI CVT".
- **Valuación**: Precio de referencia por año-modelo. Los precios se almacenan en **USD**.

## Monedas y tipo de cambio

Los precios base están en **USD**. Podés solicitar conversión a **ARS** con `currency=ars`.
La cotización se obtiene del tipo de cambio oficial (fuente: [Bluelytics](https://bluelytics.com.ar)), cacheada por 15 minutos.

Si la API de cotización no está disponible, el endpoint responde `503`.

## Paginación

Los endpoints paginados usan `simplePaginate`. Esto significa que la respuesta **no incluye `total`** (para mejor performance).
Navegá entre páginas usando `links.next` y `links.prev`.
...
MD,
    ...
    'extensions' => [
        \App\Docs\ScrambleExtension::class,
    ],
];
